function injectStyles() {
  ...
  .alert-alert { background-color: #ffd3d3; color: #8e3737; border: 1px solid #8e3737; }
  .alert-warning { background-color: #FBFFE0; color: #78842f; border: 1px solid #78842f; }
  .alert-notice { background-color: #f0feff; color: #70898a; border: 1px solid #70898a; }
...
class Alert {
  ...
  show() {
    if (
      !["primary", "success", "alert", "warning", "notice"].includes(this.state)
    ) {
      console.error(
        `Invalid state: ${this.state}. State must be one of ["primary", "success", "alert", "warning", "notice"].`
      );
      return;
    }
